...
FLASH_Status FLASH_ErasePage(uint32_t Page_Address)
{
  ...
  {
    // //  __set_PRIMASK(1); // if the system is configured intterupt,needs to be disabled all IRQ here// if the system is configured intterupt,needs to be disabled all IRQ here
    ...
    ////__set_PRIMASK(0); // enable al IRQ here // enable al IRQ here
...
FLASH_Status FLASH_EraseAllPages(void)
{
  ...
  if(status == FLASH_COMPLETE)
  {
    //  __set_PRIMASK(1); // if the system is configured intterupt,needs to be disabled all IRQ here
    ...
    /* Wait for last operation to be completed */
    status = FLASH_WaitForLastOperation(FLASH_ER_PRG_TIMEOUT);
    //__set_PRIMASK(0); // enable al IRQ here
    /* Disable the MER Bit */
    FLASH->CR &= ~FLASH_CR_MER;
  }
...
{
  uint32_t FLASH_WRPR = 0;
  /* Return the FLASH write protection Register value */
  FLASH_WRPR = *(__IO uint32_t*)(0x1FFFF808) & 0xFF;
  FLASH_WRPR |= (*(__IO uint32_t*)(0x1FFFF808) & 0xFF0000) >> 8;
  FLASH_WRPR |= (*(__IO uint32_t*)(0x1FFFF80C) & 0xFF) << 16;
  FLASH_WRPR |= (*(__IO uint32_t*)(0x1FFFF80C) & 0xFF0000) << 8;
  return FLASH_WRPR;
...
FLASH_Status EEPROM_EraseByte(uint32_t Address)
{
  ...
  { //  __set_PRIMASK(1); // if the system is configured intterupt,needs to be disabled all IRQ here
    ...
    /* must add four noop instructions here because of address alignment, otherwise hardfault interruptions may occur  */
    __NOP();
    __NOP();
    __NOP();
    __NOP();
